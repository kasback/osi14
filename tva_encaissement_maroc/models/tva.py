    ...
    def generate_data(self):
        ...
        for record in self:
            ...
            for move in move_ids:  # Pièces
                if move.journal_id.default_credit_account_id.user_type_id.type in ('liquidity', 'other'):
                    ...
                    for move_pay_line in move_pay_ids:  # Paiement
                        ...
                        if move_pay.move_id and move_pay.move_id.invoice_line_ids:  # Si le paiment appartient à une/des factures
                            ...
                            if move_pay.move_id.type in ('out_refund', 'in_refund'):  # avoirs
                                tax_dict[tax_line['id']] = [-amount_ht, -amount_tax, -amount_ttc]
                            type = False

                            if move_pay.move_id.type in ('out_invoice', 'out_refund'):
                                type = True
                                seq_enc += 1
                            else:
                                seq_dec += 1
...
#         cheque_journal = self.env.ref('account_data_ma.account_journal_data_chp')
