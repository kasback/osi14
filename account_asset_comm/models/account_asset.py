...
class AccountAssetAsset(models.Model):
    ...
    def compute_depreciation_board(self):
        ...
        if self.value_residual != 0.0:
            ...
            #     if self.method_period >= 12:
            #         asset_date = self.date.replace(day=1, month=1)
            #     else:
            ...
            for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                ...
                # month = depreciation_date.month
                # year = depreciation_date.year
                if day > 28 and self.date_first_depreciation == 'manual':
                    max_day_in_month = calendar.monthrange(depreciation_date.year, depreciation_date.month)[1]
                    depreciation_date = depreciation_date.replace(day=min(max_day_in_month, day))
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def create_grouped_move(self, post_move=True):
        ...
        for line in self:
            ...
            company_currency = line.asset_id.company_id.currency_id
            current_currency = line.asset_id.currency_id
            amount += current_currency._convert(line.amount, company_currency, line.asset_id.company_id, fields.Date.today())
